...
export async function executePanicActions(
    ...
    try {
      ...
      if (countExportableItems(backup) === 0) {
        results.downloadError =
          'Nothing to back up: no secrets files, secrets, or associated accounts were found';
      } else {
  ...
  if (flushEnvs) {
    try {
      const countBefore = await SecretFile.countDocuments({ projectId: { $in: projectIds } });
      await SecretFile.deleteMany({ projectId: { $in: projectIds } });
      results.flushEnvs = true;
      results.flushedEnvCount = countBefore;
    } catch (error) {
      console.error('Flush secrets files error:', error);
      results.flushError = error instanceof Error ? error.message : 'Failed to flush secrets files';
    }
  }
