...
function formatActivityLabel(entry: ActivityEntry): string {
  ...
  switch (resourceType) {
    case 'env':
    case 'secret_file':
      return `Secrets file ${action}${metadata?.fileName ? `: ${metadata.fileName}` : ''}${metadata?.version != null ? ` v${metadata.version}` : ''}`;
    case 'secret':
...
export default function ProjectActivityPage() {
  ...
  const resourceFilters = [
    ...
    { id: 'secret_file', label: 'Secrets files' },
  ...
  return (
    <>
      <ProjectPageHeader
        ...
        title="Activity"
        description="Timeline across secrets files, secrets, accounts, tokens, and members."
      />
