        ...
        <div className="absolute top-full left-0 mt-2 w-64 rounded-lg border border-[var(--border)] bg-[var(--surface)] shadow-lg z-50">
          ...
          <div className="border-t border-[var(--border)] p-2">
            {currentOrg.type === 'team' && (
              <>
                {(hasOrgPermission(currentOrg.role, (currentOrg.permissions ?? []) as OrgPermission[], 'org:invite') ||
                  hasOrgPermission(currentOrg.role, (currentOrg.permissions ?? []) as OrgPermission[], 'org:manage_members')) && (
                  <Link
                    href={`/organizations/${currentOrg._id}/members`}
                    onClick={() => setIsOpen(false)}
                    className="w-full flex items-center gap-2 px-2 py-2 rounded-md text-sm font-medium text-[var(--foreground)] hover:bg-[var(--surface-hover)] transition-colors mb-1"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {hasOrgPermission(currentOrg.role, (currentOrg.permissions ?? []) as OrgPermission[], 'org:update') && (
                  <Link
                ...
                {hasOrgPermission(currentOrg.role, (currentOrg.permissions ?? []) as OrgPermission[], 'org:audit') && (
                  <Link
